...
import { streamChat, chatWithCsvTools, chatWithYoutubeTools, generateImage, CODE_KEYWORDS } from '../services/gemini';
...
export default function Chat({ user, onLogout }) {
  ...
  const [sessionJsonData, setSessionJsonData] = useState(null);  // loaded JSON data
  ...
  const lastName = useMemo(() => user?.lastName || '', [user]);
  ...
  // ── YouTube download → chat link ──────────────────────────────────────────
  const handleJsonFromDownload = useCallback((json) => {
    setJsonContext(json);
    setSessionJsonData(json.data);
    setActiveTab('chat');
  }, []);
  ...
  const handleSend = async () => {
    ...
    // Mode selection:
    //   useYoutubeTools — JSON loaded → client-side YouTube tools
    ...
    const IMAGE_GEN_KEYWORDS = /\b(generate\s+(an?\s+)?image|create\s+(an?\s+)?image|make\s+(an?\s+)?image|generate\s+(a\s+)?picture|create\s+(a\s+)?picture)\b/i;
    const wantImageGen = IMAGE_GEN_KEYWORDS.test(text);
    ...
    const jsonPrefix = capturedJson
      ? `[JSON File: "${capturedJson.name}" loaded]\n\nYouTube Channel Data:\n\`\`\`json\n${JSON.stringify(capturedJson.data, null, 2).slice(0, 5000)}\n\`\`\`\n\n---\n\n`
      : sessionJsonData
      ? `[JSON Data: YouTube channel with ${sessionJsonData.videos?.length || 0} videos loaded]\n\n`
      : '';
...
`
    ...
    const promptForGemini = userContext + jsonPrefix + csvPrefix + (text || (images.length ? 'What do you see in this image?' : capturedCsv ? 'Please analyze this CSV data.' : capturedJson ? 'Please analyze this YouTube channel data.' : ''));
    ...
    try {
      if (useYoutubeTools) {
        // ── YouTube tools path: Gemini picks tool + args, JS executes ────────
        console.log('[Chat] useYoutubeTools=true | videos:', sessionJsonData?.videos?.length || capturedJson?.data?.videos?.length || 0);
        const jsonData = sessionJsonData || capturedJson?.data;
        const { text: answer, charts: returnedCharts, toolCalls: returnedCalls } = await chatWithYoutubeTools(
          history,
          promptForGemini,
          (toolName, args) => executeYoutubeTool(toolName, args, jsonData, capturedImages[0])
        );
        fullContent = answer;
        ...
        for (let i = 0; i < toolCharts.length; i++) {
          const chart = toolCharts[i];
          if (chart._chartType === 'generated_image' && chart.needsGeneration) {
            console.log('[Chat] Generating image with prompt:', chart.prompt);
            const referenceImage = capturedImages[0] || null;
            const imageResult = await generateImage(chart.prompt, referenceImage);
        ...
        console.log('[Chat] YouTube returnedCharts:', JSON.stringify(toolCharts));
        console.log('[Chat] YouTube toolCalls:', toolCalls.map((t) => t.name));
        setMessages((m) =>
          m.map((msg) =>
            msg.id === assistantId
              ? {
      ...
      } else if (wantImageGen && !useCodeExecution) {
        // ── Image generation path (no JSON loaded) ──────────────────────────
        fullContent = 'Generating image...';
        setMessages((m) =>
          m.map((msg) => (msg.id === assistantId ? { ...msg, content: fullContent } : msg))
        );
        const referenceImage = capturedImages[0] || null;
        const imageResult = await generateImage(text, referenceImage);
        if (imageResult.success) {
          toolCharts = [{
          ...
          fullContent = 'Here is the generated image!';
        } else {
          fullContent = imageResult.message || 'Image generation failed.';
        }
        setMessages((m) =>
          m.map((msg) =>
            msg.id === assistantId
    ...
    <div className="chat-layout">
      ...
      <aside className="chat-sidebar">
        ...
        <div className="sidebar-footer">
          <span className="sidebar-username">{firstName && lastName ? `${firstName} ${lastName}` : username}</span>
      ...
      <div className="chat-main">
        ...
        {activeTab === 'youtube' ? (
          <YouTubeDownload onJsonLoaded={handleJsonFromDownload} />
        ) : (
        <>
        <div
          ...
          {messages.map((m) => (
            <div key={m.id} className={`chat-msg ${m.role}`}>
              ...
              {m.charts?.map((chart, ci) => {
                ...
                } else if (chart._chartType === 'metric_vs_time') {
                  return (
                    <MetricVsTimeChart
                      key={ci}
                      data={chart.data}
                ...
                } else if (chart._chartType === 'video_card') {
                  return <VideoCard key={ci} video={chart.video} />;
                } else if (chart._chartType === 'generated_image') {
                  if (chart.generatedImage) {
                    return (
                      <div key={ci} className="generated-image-container">
                        <div className="generated-image-header">
                          <p>🎨 Generated Image</p>
                          <p className="image-prompt"><strong>Prompt:</strong> {chart.prompt}</p>
                          {chart.style && chart.style !== 'default' && (
                            <p className="image-style"><strong>Style:</strong> {chart.style}</p>
                          )}
                        </div>
                        <img
                          src={`data:${chart.generatedImage.mimeType};base64,${chart.generatedImage.data}`}
                          alt={chart.prompt}
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            link.href = `data:${chart.generatedImage.mimeType};base64,${chart.generatedImage.data}`;
                    ...
                    return (
                      <div key={ci} className="generated-image-placeholder">
                        <p>🎨 Image generation failed:</p>
                        <p className="image-gen-note">{chart.error}</p>
                      </div>
                    );
                  } else {
                    return (
                      <div key={ci} className="generated-image-placeholder">
                        <p>🎨 Image generation requested:</p>
                        <p><strong>Prompt:</strong> {chart.prompt}</p>
                        <p><strong>Style:</strong> {chart.style}</p>
                        <p className="image-gen-note">Generating...</p>
                      </div>
                    );
                  }
                }
        ...
        <div className="chat-input-area">
          ...
          {jsonContext && (
            <div className="csv-chip">
              ...
              <span className="csv-chip-meta">
                {jsonContext.data.videos?.length || 0} videos
              </span>
