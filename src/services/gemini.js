...
// Similar to CSV tools but for YouTube channel data analysis
...
// ── Image generation using Gemini 2.0 Flash native image generation ───────────
export const generateImage = async (prompt, referenceImage = null) => {
  try {
    ...
    // Add reference/anchor image if provided
    if (referenceImage) {
      parts.push({
        inlineData: {
    ...
    const imageData = responseParts.find((p) => p.inlineData?.mimeType?.startsWith('image/'));
    ...
    return {
      success: false,
      message: textParts || 'Image generation failed — no image was returned by the model.',
    };
  } catch (error) {
